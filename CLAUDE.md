...
Builds are driven through CMake presets (`CMakePresets.json`); there is no ad hoc build path. List them with
`cmake --list-presets=configure` / `--list-presets=build`.
...
`-debug`, `-release`, `-minsizerel`, `-relwithdebinfo`. Even `-cpm` presets need several system packages
pre-installed, since CPM-fetched deps still `find_package()` dependencies of their own; the relevant
`.github/workflows/ci-*.yml` has the list per platform.
...
CI's "checking that HDRView runs" step invokes `HDRView --help` on every built preset; keep it working even
when the GUI itself can't be exercised headlessly.
...
`-cpm`/`-universal` presets, not the `-local` ones. See the
`HDRVIEW_TEST_OPENEXR_DIR`/`HDRVIEW_TEST_PNG_CONTRIB_DIR` guards in `CMakeLists.txt` and those test files.
CI runs the full suite on one `-cpm`/`-universal` preset per platform (Release only), not across the whole
matrix; see `run_tests`/`HDRVIEW_BUILD_TESTS` in `.github/workflows/ci-{mac,linux,windows}.yml`. Treat a
successful build plus `ctest` as the verification bar for anything covered by these tests; for everything
else (most GUI/interaction code), fall back to a manual/CLI smoke check or the GUI tests below.

#### What to test, and where

The goal is the best coverage per second of runtime, not the largest number of tests: the suite is run
constantly, and a slow one stops being run.

- **Broaden an existing test before adding a new one.** When a bug turns up, ask which existing test
  *should* have caught it and why it didn't, usually because it was too narrow or built on an input that
  agreed with the bug, and fold the case in there. A new `TEST_CASE` per fixed bug produces a suite that is
  long, slow, and still full of holes.
- **Test the category, not the instance.** A sweep over every registered command, mapping or loader,
  asserting one property of all of them, is worth more than one test of the case that broke, and covers the
  ones nobody has broken yet.
...
- **Mutation-check every test.** Break the thing it covers, confirm it fails, and look at *how many*
...
Engine](https://github.com/ocornut/imgui_test_engine). It registers as its own `ctest` entry, separate from
`hdrview_tests`'s `doctest_discover_tests` registration, so GUI-test failures and logs never mix in with the
logic-level suite's. The option also asks hello_imgui's CPM package for `HELLOIMGUI_WITH_TEST_ENGINE ON`,
which fetches and builds the engine itself:
```bash
...
```
The binary takes no arguments: it opens a real HDRView window, runs every registered test, and exits. There
is no headless mode, since capture and input both go through a live GL context, so CI runs it under
`xvfb-run`.
...
`HDRViewApp::enable_gui_test_engine()` opt-in hook, guarded behind `HDRVIEW_ENABLE_GUI_TEST_ENGINE` (defined
only for the `hdrview_gui_tests` target) so the production `HDRView` binary and `hdrview_tests` are untouched
by it. That hook also overrides `io.ConfigRunSpeed` to `ImGuiTestRunSpeed_Fast`: Hello ImGui's test-engine
`Setup()` hardcodes `Normal` speed ("slowest mode... in this demo"), which animates every simulated mouse
move over many real frames instead of teleporting it.
...
- `WindowInfo()`/`GatherItems()`/`MenuClick()` all resolve a bare (non-`"//"`-prefixed) ref *relative to
  ...
  or the lookup silently searches the wrong scope and just doesn't find it. This is the most common mistake
  in this suite.
- A menu/action name containing a literal `/` (e.g. `"Pixel/color inspector"`) needs it escaped as `\/` in a
  `MenuClick()` path, or the `/` gets parsed as a submenu separator; this mirrors how Dear ImGui's own demo
...
- `BeginTable(...)`-hosted content (e.g. the Images-window file list) lives in a child window whose name gets
  a runtime ID-hash suffix (`"Images/ImageList_<hex>"`), not the literal table name, so don't hardcode that
...
- `HDRViewApp::load_images({pathA, pathB})` does not guarantee the resulting `m_images` order matches the
  request order: background loads can complete in either order. Tests that care which loaded image is which
...
registers zero tests otherwise, complementing the single-layer PNG fixtures (`HDRVIEW_GUI_TEST_IMAGE`/`_2`)
the rest of the suite uses. A multi-part EXR loads as one `Image` per part, not as several `ChannelGroup`s
within one `Image`; that is how a single-part *multi-layer* EXR behaves.

`tests/gui/test_gui_screenshots.cpp` rides the same harness to regenerate the README's screenshots. It
registers zero tests unless `HDRVIEW_SCREENSHOT_DIR` is set, so an ordinary `ctest` run never sees it;
`resources/regenerate-screenshots.sh` is the entry point, and `HDRVIEW_SCREENSHOT_IMAGES` points it at
subject images other than the in-tree fixtures. Capture rides hello_imgui's test-engine integration, which
installs `ImGuiApp_ImplGL_CaptureFramebuffer` as the engine's `ScreenCaptureFunc`, but only under
`#ifdef HELLOIMGUI_HAS_OPENGL`: an OpenGL-backend-only facility, which cannot produce macOS screenshots.
`enable_gui_test_engine()` also honors `HDRVIEW_SCREENSHOT_SIZE`/`_SCALE`, so the window geometry and UI
density are stated rather than inherited.
...
transfer, PQ on an HDR display), and a PNG holding linear light is read back as sRGB and looks far too dark.
So `enable_gui_test_engine()` overrides `ScreenCaptureFunc` with `capture_colorpass_framebuffer()`
(`app-windows.cpp`), which binds `HDRViewApp::capture_source()`'s FBO for the read. That target holds
HDRView's extended sRGB, already sRGB-encoded with 1.0 at SDR white, so reading it as fixed-point clamps to
[0, 1] and quantizes, which is the SDR rendition a PNG wants, with the app still rendering in HDR.
`capture_source()` returns nullptr when no color management is in force, and then the window's framebuffer
is already the sRGB one.
...
`linux-cpm` Release job, under `xvfb-run`, since GitHub's Ubuntu runners have no display server). macOS and
Windows headless GUI automation is buildable but not yet exercised in CI, a gap Hello ImGui's own CI shares
(its `Automate.yml` skips macOS).
...
`ColorGamut_`, `colormap.cpp`), so a comment spelling it the other way hides from a grep for `color`.
...
`hdrview.cpp`, the `main()` entry point). Its implementation is split by concern across files:
...
- `app-file-io.cpp` — open/save/load-folder/drag-drop plumbing (delegates decode to `imageio/`)
...
(`pixel_at_vp_pos`, `vp_pos_at_pixel`, `app_pos_at_pixel`, etc.); reuse these instead of hand-rolling
transforms.
...
is global and read at generate time, so each block undoes that afterwards; see the comments there.
`HEIC`/`AVCI` stay off in released builds for patent reasons, and `HTJ2K` encoding is off in the web build
because libheif's `FindOPENJPH.cmake` insists on an install tree.

AVIF has two AV1 decoders in play. libaom is always there, and is also the AVIF *encoder*, which dav1d cannot
be, but dav1d decodes several times faster, so `HDRVIEW_ENABLE_DAV1D` asks libheif for its dav1d plugin too
...
`colorspace.h`/`colorspace.cpp` implement gamut/transfer-function conversions (used at load time via
...
mode. `dithermatrix256.*` supplies the blue-noise dithering used when downconverting HDR to an SDR display or
file format.
...
- **Windows/Linux (GLFW+OpenGL)**: the display may instead want scRGB linear, a power curve, or PQ, possibly
  ...
  (`CustomBackground` … `BeforeSwap`), so `update_colorpass()` makes the decision once per frame up front.

`display_colorspace.{h,cpp}` is the boundary: the **only** place `wp_color_manager_v1` protocol integers
(what the GLFW fork reports) exist, translating them once into `colorspace.h`'s `TransferFunction` and
`Chromaticities`. Everything downstream, the shader included, speaks HDRView's own vocabulary, and the gamut
conversion is `color_conversion_matrix()` uploaded as a uniform. When adding display-side color handling,
extend that translation instead of plumbing protocol code points further in.
