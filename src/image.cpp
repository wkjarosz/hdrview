                           ...
                           std::atomic<bool> &canceled)
{
    try
    {
        ...
        *this    = PixelStats();
        ...
        // Box::intersect() clamps each bound against the other box without keeping min <= max, so a
        // selection that misses the channel leaves an inverted box behind. Its volume() is then negative --
        // or, with both axes inverted, positive but meaningless -- and the work ranges below are size_t, so
        // it would become an enormous count and index far outside the channel. An empty region has no
        // pixels to visit: leave both loops with nothing to do and the histograms empty.
