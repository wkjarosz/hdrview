...
HSLGradient::HSLGradient(Widget *parent) : Widget(parent) {}

Vector2i HSLGradient::preferred_size(NVGcontext *) const { return Vector2i(70, 16); }
...
void HSLGradient::draw(NVGcontext *ctx)
{
    ...
    for (int i = -1; i < 6; i++)
    {
        ...
        NVGpaint paint = nvgLinearGradient(ctx, x0, 0, x1, 0, nvgHSL((i - integer) / 6.f, m_saturation, m_lightness),
    ...
    nvgBeginPath(ctx);
    nvgRoundedRectVarying(ctx, m_pos.x() + 1, center.y() - bar_radius, 2 * bar_radius, cap, bar_radius, 0.f, 0.f,
                          bar_radius);
    ...
    nvgBeginPath(ctx);
    nvgRoundedRectVarying(ctx, m_pos.x() + 1, center.y() - bar_radius, m_size.x() - 2, cap, bar_radius, bar_radius,
    ...
    // // draw the knob
    // Vector2f knob_pos(start_x + (m_value - m_range.first) / (m_range.second - m_range.first) * (width_x), center.y()
    // + 0.5f); NVGpaint shadow_paint =
    ...
    // nvgBeginPath(ctx);
    // nvgRect(ctx, knob_pos.x() - knob_radius - 5, knob_pos.y() - knob_radius - 5, knob_radius * 2 + 10, knob_radius *
    // 2 + 10 + knob_shadow); nvgCircle(ctx, knob_pos.x(), knob_pos.y(), knob_radius); nvgPathWinding(ctx, NVG_HOLE);
