...
//! No "visible groups" scope: visibility follows the channel-filter text, so typing would change what an
//! edit covers.
struct EditSubject
{
    enum Scope : int
    ...
    //! Restrict the edit to the rectangular selection. Ignored while the selection is empty, so an edit
    //! then covers the whole image.
...
//! The color groups of \p img that \p subject's scope names, and every channel of them. Only RGB and
//! RGBA groups: depth, motion vectors and the like are left alone.
