...
//! What a filter reads when it reaches past the edge of a channel. Chosen per axis, since an image can
//! tile along one and not the other.
enum BorderMode : int
{
...
//! How a filter reconstructs between samples. All three agree at whole-sample offsets; bicubic keeps
//! small detail sharper than bilinear, at the cost of a slight overshoot at hard edges.
enum Sampler : int
{
    Sampler_Nearest = 0,
    Sampler_Bilinear,
...
//! Blur separably with a Gaussian of the given standard deviations, in samples; a sigma of zero leaves
//! that axis alone. O(radius) per sample, with a truly sampled Gaussian kernel; see
//! fast_gaussian_blurred() for the cheaper approximation.
...
//! (the running sum adds the sample entering the box and subtracts the one leaving). Iterating widens the
//! blur; to approximate a Gaussian of a stated width instead, use fast_gaussian_blurred().
...
//! Median over the disc (or square, if !\p disc) of radius \p radius around each sample, which removes
//! fireflies without smearing them. No separable form, so O(radius^2) per sample and it reports \p progress.
Array2Df median_filtered(const Array2Df &src, const Box2i &region, float radius, bool disc = true,
                         AtomicProgress progress = {});

//! Replace every non-finite sample with the median of its finite neighbors, leaving finite samples alone
//! however extreme. \p replacement is used only where a sample has no finite neighbor at all.
Array2Df zapped_gremlins(const Array2Df &src, const Box2i &region, float replacement = 0.f);

//! Sharpen by adding back \p amount times what a blur of \p sigma removed: 0 leaves the image alone, 1
//! doubles that detail. Small \p sigma sharpens fine texture, large raises local contrast.
