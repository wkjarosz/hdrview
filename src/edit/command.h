...
struct EditContext
{
    ...
    //! The channel groups an operation acts on: normally the selected ones, but see
    //! HDRViewApp::target_groups() for how a right-clicked group overrides that. Empty when there are none.
    ...
    //! Put \p img into the image list beside the one being edited, and select it. \p partname is what the
    //! Images panel shows beside the file name, to tell several images made from one file apart.
...
//! One undoable edit: what it is called, what it draws, and what it does. Add one by writing a subclass
//! and naming it in the table in commands.cpp.
/*!
...
public:
    ...
    struct Info
    {
        ...
        //! Least width of the dialog, in em; content wider than this widens it further. In em because an
        //! Info is built before there is an ImGui context to ask for a size.
        float width_em = 24.f;
        //! Whether the "Apply to" settings mean anything for this edit, and so whether its dialog shows
        //! them. False for the edits that replace or reshape the whole image.
        bool draws_subject_selector = true;

        //! With several images selected, whether this runs on all of them or only the current one. Each
        //! gets its own invocation and undo entry.
        bool fans_out = true;
