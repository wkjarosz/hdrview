...
ColorSlider::ColorSlider(Widget *parent, const Color & c, ColorMode m)
    : Slider(parent), m_color(c), m_mode(m)
{
    
}

Vector2i ColorSlider::preferred_size(NVGcontext *) const
{
    return Vector2i(70, 16);
}

bool ColorSlider::mouse_drag_event(const Vector2i &p, const Vector2i & /* rel */,
...
void ColorSlider::draw(NVGcontext* ctx)
{
    ...
    nvgBeginPath(ctx);
    nvgRoundedRect(ctx, start_x - bar_radius, center.y() - bar_radius, width_x + 2*bar_radius, 2 * bar_radius + 1, bar_radius);
