...
/// piecewise-linear fit published in Apple's developer forums. The defaults are the weakest reconstruction.
...
/// Bilinearly resample \p gainmap to \p size, the base image's resolution, or return an empty map if it is
/// malformed. \p linearize undoes an sRGB encoding on the way: Apple's maps are sRGB-encoded, ISO 21496-1
/// maps carry their own and must not be.
...
//! to them. Alpha is not part of what the map converts and is not copied.
...
//! Flatten a decoded image's color channels into a gain map, stopping at three since a gain map is
//! monochrome or RGB. Empty if \p map has no channels or they disagree on size.
...
struct IsoGainmapParams
{
    ...
    //! How much of the map to apply to land at \p target_stops of headroom: 0 leaves the base rendition
    //! alone, 1 applies the map in full, and negative applies it in reverse when the alternate is darker.
...
//! Parse Adobe's `hdrgm:` XMP schema, which encodes the same parameters as ISO 21496-1. Returns nullopt
//! when the packet carries no `hdrgm:` properties, and throws when one is present but a defaultless
//! property is missing.
...
/// itself, decoded out of its gamma and normalization so the group reads in log2 gain. \p target_stops is
/// the headroom to land at, k_full_gainmap_headroom for the alternate rendition in full; \p keep_renditions
/// also keeps the file's own base rendition and gain map as their own channel groups.
...
/// itself, which happens whether or not it is applied so it stays inspectable at \p target_stops of zero.
