...
std::function<void()> fill_callback(const Color &nfg, ImageListPanel *images_panel)
{
    return [nfg, images_panel]()
    {
        ...
        images_panel->async_modify_selected(
            [nfg](const ConstHDRImagePtr &img, const ConstXPUImagePtr &xpuimg) -> ImageCommandResult
            {
                ...
                return {
                    make_shared<HDRImage>(img->apply_function([&fg](const Color4 &c) { return fg; }, xpuimg->roi())),
                    nullptr};
            });
    };
}
