...
void Texture::init()
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        default: throw std::invalid_argument("Texture::Texture(): invalid interpolation mode!");
        }
    }
...
void Texture::upload(const uint8_t *data)
{
    if (m_samples > 1 && data != nullptr)
        throw std::invalid_argument("Texture::upload(): only implemented for samples=1!");
...
{
    if (m_samples > 1 && data != nullptr)
        throw std::invalid_argument("Texture::upload_sub_region(): only implemented for samples=1!");
...
int Texture::max_size()
{
    ...
    {
#if defined(HELLOIMGUI_USE_GLAD)
        // glGetIntegerv is a GLAD function pointer that stays null until gladLoadGL() has run (e.g. no window
        // was ever created, as in headless unit tests), so report "no limit" instead of crashing
        if (!glGetIntegerv)
            return std::numeric_limits<int>::max();
#endif
        glGetIntegerv(GL_MAX_TEXTURE_SIZE, &s_max_size);

        // Without a current context glGetIntegerv leaves s_max_size at 0; report "no limit" and retry on the
        // next call instead of caching 0 and rejecting every image. Image::finalize() checks every image, so
        // anything too large is still caught by the first call that does have a context.
        if (s_max_size == 0)
            return std::numeric_limits<int>::max();
    }
...
{
    // trace, not info: a streaming image regenerates its mip chain on any frame a tile landed on
                                  ...
                                  GLenum &pixel_format_gl, GLenum &component_format_gl, GLenum &internal_format_gl)
{
    ...
    if (component_format_gl == 0)
        throw std::invalid_argument("gl_map_texture_format(): invalid component format!");
    if (pixel_format_gl == 0)
        throw std::invalid_argument("gl_map_texture_format(): invalid pixel format!");
    if (internal_format_gl == 0)
        throw std::invalid_argument(
            "gl_map_texture_format(): component format unsupported for the given pixel format!");
}
