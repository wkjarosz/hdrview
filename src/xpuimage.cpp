...
void XPUImage::async_modify(const ImageCommandWithProgress & command)
{
	...
	m_async_command = make_shared<ModifyingTask>([this,command](AtomicProgress & prog){return command(m_image, prog);});
...
void XPUImage::recompute_histograms(float exposure) const
{
    ...
    {
        m_histograms = make_shared<HistogramTask>(
	        [this,exposure](AtomicProgress & prog)
	        {
