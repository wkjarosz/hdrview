    ...
    m_render_pass(render_pass), m_name(name), m_blend_mode(blend_mode), m_shader_handle(0)
{
    ...
    auto register_buffer = [&](BufferType type, const std::string &name, int index, GLenum gl_type)
    {
        if (m_buffers.find(name) != m_buffers.end())
            throw std::invalid_argument("Shader::Shader(): duplicate attribute/uniform name in shader code!");
        ...
        default: throw std::invalid_argument("Shader::Shader(): unsupported uniform/attribute type!");
        };
...
void Shader::begin()
{
    ...
    for (auto &[key, buf] : m_buffers)
    {
        ...
        case VertexBuffer:
            ...
            default: throw std::invalid_argument("Shader::begin(): unsupported vertex buffer type!");
            }

            if (buf.ndim != 2)
                throw std::invalid_argument("\"" + m_name + "\": vertex attribute \"" + key +
                                            "\" has invalid dimensions (expected ndim=2, got " +
        ...
        case UniformBuffer:
            ...
            if (uniform_error)
                throw std::invalid_argument("\"" + m_name + "\": uniform attribute \"" + key +
